...
def adicionar(descricao, extras):
  ...
  if descricao == "" or novaAtividade == " "*(len(novaAtividade)):
    print("mensagem de erro: ausência de descrição ou pusestes vários espaços vazios")
    return False
  try:
    fp = open(TODO_FILE, "a") #encoding='utf-8-sig')#deixar comentado caso erro se repita
...
def listar():
    ...
    for x in atividades:
      ...
      if pri == "(A) " or pri == "(a) ":
        ...
        printCores(x, GREEN)
      else:# pri != "(A) " and pri != "(B) " and pri != "(C) " and pri != "(D) ":
        print(x)
...
def fazer(num):
  ...
  i = 1
  numValido = True
  if num not in range(1,len(atividades)+1):
    print("mensagem de erro, num não corresponde a nenhuma atividade")
    numValido = False
  if numValido:
    for x in atividades:
...
def priorizar(num, prioridade):
  ...
  if prioridadeValida(prioridade) == False:
    ...
    atividades = ordenarPorPrioridade(atividades)
    atividades = ordenarPorDataHora(atividades)
    if num not in range(len(atividades)+1):
      print("mensagem de erro: atividade não existe")
    i = 0
    for x in range(len(atividades)):
      if x+1 != num:
        linhas3.append(atividades[i][1][0] + " " + atividades[i][1][1] + " " + atividades[i][1][2] + " " + atividades[i][0] + " " + atividades[i][1][3] + " " + atividades[i][1][4])
      else:
        linhas3.append(atividades[i][1][0] + " " + atividades[i][1][1] + " " + prioridade.upper() + " " + atividades[i][0] + " " + atividades[i][1][3] + " " + atividades[i][1][4])
      i = i + 1
    arqv = open(TODO_FILE, "w") #encoding = 'utf-8-sig')
    for x in linhas3:
       x = x.strip()
...
def processarComandos(comandos):
  if comandos[1] == ADICIONAR:
    ...
    if len(comandos) != 4:
      print("input incorreto. mensagem de erro")
    else:
      priorizar(int(comandos[2]),comandos[3])
    
  else :
    print("Comando inválido.")
  #print(organizar([' '.join(comandos)])) #uma lista com um item correspondente a linha organizada
