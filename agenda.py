...
def fazer(num):
  ...
  for x in range(len(linhas1)):
    if x == num:
      linhaPop = linhas1.pop(i)
...
def priorizar(num, prioridade):
  if ValidarPrioridade(prioridade) == False:
    print("Prioridade inválida")
  else:
    existirPrioridade = True
    linhas3 = []
    arqv = open(TODO_FILE, "r", encoding='utf-8-sig')
    linhas = arqv.readlines()    
    arqv.close()
    ...
    for x in range(len(atividades)):
      if x != num:
        ...
        if atividades[x][1][2] == "":
          existirPrioridade = False
          print("mensagem de erro: não existe prioridade")
      if existirPrioridade:
        arqv = open(TODO_FILE, "w", encoding = 'utf-8-sig')
        for x in linhas3:
          arqv.write(x + "\n")
        arqv.close()
